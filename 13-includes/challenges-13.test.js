...
const firstLetters = (strs) => {
  ...
  return strs.map(value => value.charAt(0));
};
...
const findHappiness = (strs) => {
  ...
  return strs.filter(value => value.includes(':)'))
...
const standardizePhoneNumbers = (phoneNumbers) => {
  // Solution code here...
  // let output = [];
  // return phoneNumbers.((val, idx) => {
  //   val.forEach((num, idx2) => {
  //     (num.match(/\d/)) ? output.push(num[idx2])}
...
const onlyOddChars = (str) => {
  ...
  //   str.forEach(val, idx => (idx % 2) ? output += val);
};
...
const allHappy = (strs) => {
  return strs.map(value => value.includes(':)'));
}
