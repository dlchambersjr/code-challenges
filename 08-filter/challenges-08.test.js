...
const oddValues = (input) => {
  // Solution code here...
  return input.filter(odd => odd % 2 !== 0);
}
...
const filterStringsWithVowels = (input) => {
  // Solution code here...
  return input.filter(vowels => vowels.match(/[aeiou]/gi));
};
...
const notInFirstArray = (forbiddenValues, input) => {
  // Solution code here...
  let newArray = [];
  forbiddenValues.forEach(verboten => {
    console.log(verboten);
    newArray = input.filter(value => {
      console.log(value);
      return value !== verboten;
    })
  })
