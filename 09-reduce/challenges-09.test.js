...
const countNumberOfElements = (input) => {
  // Solution code here...
  let arrayLength = input.reduce(function (acc, val, idx) {
    acc++;
    return acc;
  }, 0);
  return arrayLength;
...
const countNumberOfChildren = (input) => {
  ...
  let numberKids = input.reduce(function (acc, val, idx) {
    if (val.children && val.children.length > 0) { acc += val.children.length }
    return acc;
  }, 0);
  return numberKids;
};
